...
const FairyGlow = styled.div`
  ...
  filter: blur(20px);
  animation: ${fairyGlow} 2s ease-out forwards;
  z-index: 1;
`;

const FairyTrail = styled.div`
  position: absolute;
  top: 50%;
  left: 50%;
  width: 200px;
  ...
  filter: blur(10px);
  animation: ${fairyTrail} 2s ease-out forwards;
  z-index: 1;
`;
...
function Header() {
  ...
  useEffect(() => {
    // Hide fairy after animation completes
